...
const BmiCalculator = () => {
  const getHeight = JSON.parse(localStorage.getItem('height'))
  const getWeight = JSON.parse(localStorage.getItem('weight'))
  const [height, setHeight] = useState(getHeight !== null ? getHeight : 170)
  const [weight, setWeight] = useState(getWeight !== null ? getWeight : 60)
  ...
  useEffect(() => {
    ...
    console.log('Height is stored in local storage')
  }, [height])

  useEffect(() => {
    localStorage.setItem('weight', JSON.stringify(weight))
    console.log('weight is stored in local storage')
  }, [weight])
